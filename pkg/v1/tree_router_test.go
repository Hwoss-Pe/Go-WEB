...
//		handler := NewHandlerBasedOnTree().(*HandlerBasedOnTree)
//		handler.Route(http.MethodPost, "/user", func(c *Context) {})
//		_, found := handler.root.findRouter("/user")
//		assert.True(t, found)
//		_, found = handler.root.findRouter("/user/profile")
...
//
//		handler.Route(http.MethodPost, "/order/detail", func(c *Context) {})
//		_, found = handler.root.findRouter("/order")
//		assert.False(t, found)
//
//		_, found = handler.root.findRouter("/order/detail")
//		assert.True(t, found)
//
//		handler.Route(http.MethodPost, "/order", func(c *Context) {})
//		_, found = handler.root.findRouter("/order")
...
func TestHandlerBasedOnTree_findRouter(t *testing.T) {
	...
	_ = handler.Route(http.MethodPost, "/order/detail", handleFunc(detailHandler))
	_, found = handler.findRouter(http.MethodPost, "/order", ctx)
	assert.False(t, found)

	fn, found = handler.findRouter(http.MethodPost, "/order/detail", ctx)
	assert.True(t, found)
	assert.True(t, handlerFuncEquals(detailHandler, handlerFunc(fn)))

	var wildcardHandler handlerFunc = func(c *Context) {}
	_ = handler.Route(http.MethodPost, "/order/*", handleFunc(wildcardHandler))
	_, found = handler.findRouter(http.MethodPost, "/order", ctx)
	assert.False(t, found)
	...
	assert.True(t, found)
	assert.True(t, handlerFuncEquals(wildcardHandler, handlerFunc(fn)))

	_, found = handler.findRouter(http.MethodGet, "/order/checkout", ctx)
	assert.False(t, found)

	// 参数路由
	handler.Route(http.MethodPost, "/order/*", handleFunc(wildcardHandler))
}
